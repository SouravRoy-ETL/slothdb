"""Q15 layer diagnostics — break down where time goes."""
...
QS = {
    ...
    "Q15_no_search_filter": "SELECT SearchEngineID, SearchPhrase, COUNT(*) AS c FROM 'DATA' GROUP BY SearchEngineID, SearchPhrase ORDER BY c DESC LIMIT 10",
    "Q15_no_int_proj":     "SELECT SearchPhrase, COUNT(*) AS c FROM 'DATA' WHERE SearchPhrase <> '' GROUP BY SearchPhrase ORDER BY c DESC LIMIT 10",
    "Q15_no_str_proj":     "SELECT SearchEngineID, COUNT(*) AS c FROM 'DATA' WHERE SearchPhrase <> '' GROUP BY SearchEngineID ORDER BY c DESC LIMIT 10",
    "Q15_count_only":      "SELECT COUNT(*) FROM 'DATA' WHERE SearchPhrase <> ''",
}
def t(exe, sql, n=3):
    times = []
    for _ in range(n):
