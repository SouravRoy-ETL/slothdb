...
<h3>A DuckDB competitor, built from scratch in C++20. Up to 5× faster where it counts.</h3>

<p>SlothDB is an embedded columnar SQL OLAP engine. Built from the ground up - not a fork, not a wrapper, not a dialect layer - to be a faster, lighter, more portable alternative to DuckDB. On our 5-query warm JOIN batch: <b>138 ms vs DuckDB 1.1.3's 540 ms (3.9×)</b>. On 1 M-row Avro <code>SUM</code>: <b>140 ms vs 760 ms (5.43×)</b>. 16-query suite range 1.04×-5.43× (median 1.70×, 15 wins + 1 tie). Seven file-format readers in the core binary (CSV, Parquet, JSON, Avro, Excel, Arrow, SQLite - no <code>INSTALL</code>/<code>LOAD</code> step) vs DuckDB's three. Sub-1 MB edge-WASM build that fits under Cloudflare Workers' 1 MB cap. <code>CREATE LIVE VIEW</code> with incremental append on growing CSVs. Stable C ABI with numeric error codes. MIT. Reproduce: <code>pip install slothdb &amp;&amp; python -c "import slothdb; slothdb.demo()"</code>.</p>

<p>Familiar shape if you've used DuckDB - <code>db.sql(...).fetchdf()</code> works in Python, SQL is ANSI with DuckDB-style extensions (QUALIFY, MERGE, recursive CTEs) - but the engine underneath is a separate C++20 codebase with its own columnar storage, its own Parquet / CSV / JSON / Avro / Excel / Arrow / SQLite readers, its own vectorized executor, and its own benchmark numbers. Also included: <a href="docs/ASK.md"><code>.ask</code></a>, an optional natural-language sub-REPL in the shell - off by default, rules parser only unless you build with <code>-DSLOTHDB_ASK_MODEL=ON</code>.</p>
