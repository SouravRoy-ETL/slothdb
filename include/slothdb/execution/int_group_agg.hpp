...
// Specialized aggregator for a single INT32-column GROUP BY (low
// cardinality) with aggregates limited to COUNT(*), SUM/AVG of an int
// column, and COUNT(DISTINCT) of an int column.
//
// The generic FUSED-GENERIC per-row per-agg dispatch loop in
// physical_planner.cpp costs ~90 ns/row on this shape (profiled at ~1.2 s
// of consume out of a ~1.85 s wall). This path hardcodes the agg kinds
...
// mega-group (the busiest group key) is not the single-worker long pole.
//
...
enum class IntGroupAggKind : uint8_t { CountStar, Sum, CountDistinct };
...
public:
    ...
    IntGroupAgg(int max_threads, std::vector<IntGroupAggKind> kinds);
    ~IntGroupAgg();
    IntGroupAgg(const IntGroupAgg&) = delete;
    IntGroupAgg& operator=(const IntGroupAgg&) = delete;
